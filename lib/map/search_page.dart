...
class SearchPageState extends State<SearchPage> {
  ...
  List<dynamic> autoWordList;
  // 검색어 포함 리스트
  List<String> _containedWordList;
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home: Scaffold(
                ...
                title: Stack(
                  ...
                  children: <Widget>[
                    ...
                    Container(
                      ...
                      child: TextField(
                        style: TextStyle(letterSpacing: 0.8,fontSize: _uiCriteria.textSize1, color: Colors.white,),
                        ...
                        decoration: InputDecoration(
                            ...
                            hintStyle: TextStyle(letterSpacing: 0.8,fontSize: _uiCriteria.textSize1, color: greyB3B3BC, height: 1.5),
                        ...
                        onChanged: (String value) {
                          ...
                          if (value.length > 0) {
                            ...
                            if (_getContainedWord(value).isNotEmpty) {
                              _containedWordList = _getContainedWord(value);
                            }
                            else {
                              setState(() {
                                _isContains = false;
              ...
              body: FutureBuilder(
                future: _getAutoWordList(),
                builder: (BuildContext context, AsyncSnapshot snapshot) {
                  if (snapshot.hasData) {
                    autoWordList = snapshot.data;
                                  ...
                                  return Container(
                                      ...
                                      child: Row(
                                          ...
                                          children: <Widget>[
                                            ...
                                            //       padding: EdgeInsets.symmetric(horizontal: _uiCriteria.screenWidth * 0.0186),
                                            //       decoration: BoxDecoration(
                                            //           color: (_bookmark)?Colors.white:mainColor,
                                            //           border: (_bookmark)
                                            //               ? Border.all(color: Colors.transparent)
                                            //               : Border.all(color: greyD8D8D8, width: 0.5),
                                            //           borderRadius:
                                            ...
                                            //               fontWeight: FontWeight.w500,
                                            //               color:
                                            //               (_bookmark)?mainColor:Colors.white,
                                            //               fontSize: _uiCriteria.screenWidth * 0.027
                                            //           )),
                                            //     ),
                                            //     onTap: () {
  ...
  ///자동완성 리스트를 서버로부터 가져옴
  Future<List<dynamic>> _getAutoWordList() async {
    var url = Uri.parse("http://icnogari96.cafe24.com:8080/split/zone/auto/complete");
 ...
 void addPostWord(String word) async {
  ...
  List<String> _getContainedWord(String value) {
    // 검색어를 포함하는 리스트 생성
    List<String> contain = [];
    for (int i = 0; i < autoWordList.length; i++) {
      if (autoWordList[i].contains(value)) {
        contain.add(autoWordList[i]);
        setState(() {
          _isContains = true;
        });
      }
  ...
  List<Widget> _createAutoTileList(List<String> list) {
